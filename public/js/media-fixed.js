// Media Fixed v1.0 — iOS-friendly photo capture + safer audio previews
...
const mediaFixed = (() => {
    ...
    return {
        async capturePhotoFlow() {
            // If camera not supported – fallback to file input handled elsewhere
            ...
            try {
                ...
                if (err.message !== 'camera-not-supported') {
                    alert('Не удалось открыть камеру. Разрешите доступ или выберите файл.');
                }
                throw err;
            } finally {
